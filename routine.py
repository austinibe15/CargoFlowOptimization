...
def estimate_offloaded_containers(total_containers, expected_crane_time, expected_agv_time):  
    # Simulate offloading capability based on available time  
    # Assuming total time available for processing is calculated from the expected times  
    total_processing_time = expected_crane_time + expected_agv_time  # Total expected time per cycle  
    processing_capacity = N_containers / total_processing_time  # Containers processed per minute  
...
if __name__ == "__main__":  
    ...
    # Output results  
    print(f"Expected AGV Travel Time: {expected_agv_time:.2f} minutes")  
    print(f"Expected Crane Handling Time: {expected_crane_time:.2f} minutes")  
    print(f"Estimated Number of Containers that can be Offloaded: {offloaded_containers} containers")